  ...
  try {
    const response = await client.chat.send({
      chatRequest: {
        ...
        // Рассуждение (reasoning) у deepseek-v4.1-flash включено по умолчанию и съедает
        // ~85% выходных токенов (10k на кусок из 6 событий вместо 1-2k) — контенту JSON
        // оно не нужно. Выключаем effort:'none' (замер — docs/lucid/MODELS.md, «Замер 3»)
