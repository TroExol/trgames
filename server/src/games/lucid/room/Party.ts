...
export class Party {
  ...
  // раз, и сбрасываем всё, что успел выставить start. Состав и предложенные
  // темы не трогаем — их собирал не start. Расход на генерацию откатывать
  // не нужно вовсе: строки в generation_calls/generations пишутся сразу по
  // ходу генерации (см. onCall и запись итога в init.ts), а не хранятся
  // в Party до отката. Если генерация уже доехала до PLAYING, откатывать
  // нечего: сбой пришёлся на что-то после старта, и фазу менять нельзя
  public abortStart = (): void => {
    ...
    // start присваивает state одной инструкцией вместе с переходом в
    // PLAYING, без await между ними, так что застать здесь уже выставленный
    // state нельзя — но если порядок в start когда-нибудь изменится, откат
    // не должен молча унести чужое состояние
