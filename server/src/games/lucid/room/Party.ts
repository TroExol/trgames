...
export class Party {
  ...
  // темы не трогаем — их собирал не start. Usage не трогаем тоже: если
  // generate успел вернуться, потраченное должно сохраниться при persist,
  // а не потеряться вместе с остальным откатом. Если генерация уже доехала
  // до PLAYING, откатывать нечего: сбой пришёлся на что-то после старта,
  // и фазу менять нельзя
