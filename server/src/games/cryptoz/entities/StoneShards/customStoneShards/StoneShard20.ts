...
export class StoneShard20 extends AbstractStoneShard {
  constructor(room?: Room) {
    super({ id: 20, room });
  }

  protected getDescription = () => t('cryptoz.stoneShards.stoneShard20.description', 'ru');
  ...
  protected onChangeOwner = (prevOwner: Player | null) => {
    ...
    this.owner.modifiersPrice
      .addModifier(new PriceModifier(this.readableId, (currentValue: number, card: AbstractCard) => {
        if ([CryptozShared.ECardType.WICKEDNESS, CryptozShared.ECardType.RITUAL].includes(card.type)) {
