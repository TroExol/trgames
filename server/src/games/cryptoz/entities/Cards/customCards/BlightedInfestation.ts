...
export class BlightedInfestation extends AbstractCard {
  constructor(room?: Room) {
    super({
      id: CryptozShared.ECardId.BLIGHTED_INFESTATION,
      target: CryptozShared.ECardTarget.ENEMY,
      type: CryptozShared.ECardType.ARTIFACT,
      name: t('cryptoz.cards.blightedInfestation.name', 'ru'),
      price: 4,
      baseEssence: 1,
      gloryShards: 1,
      isSeal: false,
  ...
  protected getDescription = (isSimple: boolean): CryptozShared.TCard['description'] => ({
    ...
    strike: t('cryptoz.cards.blightedInfestation.description.strike', 'ru', {
      count: this.getDamage(3, isSimple ? null : this.owner),
    }),
  });
  ...
  protected playStrikeHandler = async ({
    ...
    const damage = concreteDamage ?? this.getDamage(3, isForChaos ? null : player, target);
