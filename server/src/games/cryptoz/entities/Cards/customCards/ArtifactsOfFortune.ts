...
export class ArtifactsOfFortune extends AbstractCard {
  constructor(room?: Room) {
    super({
      id: CryptozShared.ECardId.ARTIFACTS_OF_FORTUNE,
      target: CryptozShared.ECardTarget.UNDEFINED,
      type: CryptozShared.ECardType.CREATURE,
      name: t('cryptoz.cards.artifactsOfFortune.name', 'ru'),
      price: 4,
      baseEssence: 0,
      gloryShards: 1,
      isSeal: false,
  ...
  protected getDescription = (isSimple: boolean): CryptozShared.TCard['description'] => ({
    general: t('cryptoz.cards.artifactsOfFortune.description.general', 'ru', {
  ...
  protected playGeneralHandler = async (params: TCardPlayGeneralHandlerParams) => {
    ...
    const selected = await this.room.socketService.selectCards({
      ...
      title: t('cryptoz.modals.title.takeArtifactsFromDiscard', 'ru', {
        count: 1,
      }),
    });
