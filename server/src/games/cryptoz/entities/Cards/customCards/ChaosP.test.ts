...
describe('ChaosP', () => {
  ...
  beforeEach(() => {
    ...
    card = new ChaosP(room);
  });

  it('Инстанс создается', () => {
    const card = new ChaosP();
    expect(card).toBeDefined();
    expect(card.uuid).toBeDefined();
    expect(card.id).toBe(CryptozShared.ECardId.CHAOS_P);
  ...
  describe('Хаос эффект', () => {
    it('Наносит урон только участникам с 10+ здоровья', async () => {
      // Устанавливаем разное здоровье участникам
      activePlayer.health = 15; // Получит урон
      player.health = 8; // Не получит урон
      thirdPlayer.health = 12; // Получит урон
      ...
      // Проверяем, что урон нанесен только участникам с 10+ здоровья
      expect(activePlayer.takeDamage).toHaveBeenCalledWith(5);
      expect(player.takeDamage).not.toHaveBeenCalled();
      expect(thirdPlayer.takeDamage).toHaveBeenCalledWith(5);
    });
    ...
    it('Работает с конкретными целями', async () => {
      ...
      // Проверяем, что урон нанесен только выбранным участникам
      expect(activePlayer.takeDamage).toHaveBeenCalledWith(5);
