...
export class ChaosZ extends AbstractCard {
  constructor(room?: Room) {
    super({
      id: CryptozShared.ECardId.CHAOS_Z,
      target: CryptozShared.ECardTarget.UNDEFINED,
      type: CryptozShared.ECardType.CHAOS,
      name: t('cryptoz.cards.chaosZ.name', 'ru'),
      price: 0,
      baseEssence: 0,
      gloryShards: 0,
      isSeal: false,
  ...
  protected getDescription = (): CryptozShared.TCard['description'] => ({
    strike: t('cryptoz.cards.chaosZ.description.strike', 'ru'),
  });
  ...
  protected playStrikeHandler = async ({
    ...
    await Promise.allSettled(targets.array.map(async player => {
      ...
      player.discardSeal(player.seals);
    }));
