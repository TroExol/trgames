...
import { Rootshade } from './Rootshade';
...
describe('Rootshade', () => {
  ...
  beforeEach(() => {
    ...
    player = mocks.player;
    card = new Rootshade(room);
    addCardToPlayerHand(card, activePlayer);
  });

  it('Инстанс создается', () => {
    const card = new Rootshade();
    expect(card).toBeDefined();
    expect(card.uuid).toBeDefined();
    expect(card.id).toBe(CryptozShared.ECardId.ROOTSHADE);
    expect(card.name).toBe('Тьма Корней');
  ...
  it('Описание корректное', () => {
    expect(card.format().description).toEqual({
      general: '+2 эссенции',
      strike: 'Нанеси 7 урона противнику',
  ...
  it('Разыгрывается укрытие', async () => {
    ...
    const cardEvade = new Rootshade(room);
