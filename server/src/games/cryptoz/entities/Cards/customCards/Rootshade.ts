...
export class Rootshade extends AbstractCard {
  constructor(room?: Room) {
    super({
      id: CryptozShared.ECardId.ROOTSHADE,
      target: CryptozShared.ECardTarget.ENEMY,
      type: CryptozShared.ECardType.COMPANION,
      name: t('cryptoz.cards.rootshade.name', 'ru'),
      price: 6,
      baseEssence: 2,
      gloryShards: 2,
      isSeal: false,
  ...
  protected getDescription = (isSimple: boolean): CryptozShared.TCard['description'] => ({
    ...
    evade: t('cryptoz.cards.rootshade.description.evade', 'ru'),
  });
  ...
  protected playStrikeHandler = async ({
    ...
    const damage = concreteDamage ?? this.getDamage(7, isForChaos ? null : player, target);
