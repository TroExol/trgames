...
export class BaronRattinghamOfTenebria extends AbstractCard {
  constructor(room?: Room) {
    super({
      id: CryptozShared.ECardId.BARON_RATTINGHAM_OF_TENEBRIA,
      target: CryptozShared.ECardTarget.UNDEFINED,
      type: CryptozShared.ECardType.COMPANION,
      name: t('cryptoz.cards.baronRattinghamOfTenebria.name', 'ru'),
      price: 6,
      baseEssence: 2,
      gloryShards: 2,
      isSeal: false,
  ...
  protected getDescription = (isSimple: boolean): CryptozShared.TCard['description'] => ({
    ...
    strike: t('cryptoz.cards.baronRattinghamOfTenebria.description.strike', 'ru', {
      count: this.getDamage(2, isSimple ? null : this.owner, undefined),
    }),
    evade: t('cryptoz.cards.baronRattinghamOfTenebria.description.evade', 'ru'),
  });
  ...
  public canPlayStrikeHandler = ({ tempPlayer }: TCardPlayStrikeHandlerParams) => {
    ...
    return !!new CardGroup(ECardGroupType.ANY, [...player.discard.array, ...player.arena.array])
      .getCountCardsByType(CryptozShared.ECardType.CREATURE);
  };
