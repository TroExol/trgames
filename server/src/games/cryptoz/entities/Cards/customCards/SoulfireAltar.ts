...
export class SoulfireAltar extends AbstractCard {
  private isPlayedOnTurn = false;

  constructor(room?: Room) {
    super({
      id: CryptozShared.ECardId.SOULFIRE_ALTAR,
      target: CryptozShared.ECardTarget.UNDEFINED,
      type: CryptozShared.ECardType.CRYPT,
      name: t('cryptoz.cards.soulfireAltar.name', 'ru'),
      price: 5,
      baseEssence: 0,
      gloryShards: 1,
      isSeal: true,
  ...
  protected getDescription = (): CryptozShared.TCard['description'] => ({
    seal: t('cryptoz.cards.soulfireAltar.description.seal', 'ru'),
  });
  ...
  protected playSealHandler = (params: TCardPlaySealHandlerParams) => {
    ...
    this.owner.triggersOnCardPlayed.addTrigger(new CardPlayedTrigger(this.readableId, card => {
      if (card.type === CryptozShared.ECardType.WICKEDNESS && !this.isPlayedOnTurn) {
