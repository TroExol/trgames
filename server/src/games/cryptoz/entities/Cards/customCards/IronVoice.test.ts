...
import { IronVoice } from './IronVoice';
...
describe('IronVoice', () => {
  ...
  beforeEach(() => {
    ...
    card = new IronVoice(room);
    addCardToPlayerHand(card, activePlayer);
  });

  it('Инстанс создается', () => {
    const card = new IronVoice();
    expect(card).toBeDefined();
    expect(card.uuid).toBeDefined();
    expect(card.id).toBe(CryptozShared.ECardId.IRON_VOICE);
    expect(card.name).toBe('Железный Голос');
  ...
  it('Разыгрывается укрытие и уничтожается карта с руки', async () => {
  ...
  it('Разыгрывается укрытие и уничтожается карта из сброса', async () => {
  ...
  it('Разыгрывается укрытие и не уничтожается карта', async () => {
    const handCard = new MockCard({ room });
    addCardToPlayerHand(handCard, activePlayer);
