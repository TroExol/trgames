...
## Задача 18: Края мира в генерации

Решено заказчиком 2026-09-22, после того как он увидел поле и сравнил его с настольной бродилкой, которая ему нравится. Разбор — в `docs/lucid/OPEN-QUESTIONS.md`.

На бумажной доске цвет клетки говорит, что на ней будет. У нас содержимое клетки — секрет до того, как на неё встали, поэтому красить клетки по типу события нельзя: это слило бы карту. Разнообразие цвета берётся из другого источника — **краёв мира**, которые придумывает та же модель, что придумывает мир.

**Файлы:**
- Изменить: `tools/shared/src/games/lucid/types/content.ts`
- Изменить: `server/src/games/lucid/generation/schema.ts`
- Изменить: `server/src/games/lucid/generation/prompt.ts`
- Изменить: `server/src/games/lucid/generation/fallback.json`
- Изменить: `server/src/games/lucid/generation/schema.test.ts`

- [ ] **Шаг 1: Тип края**

В `tools/shared/src/games/lucid/types/content.ts` добавить перед `TTheme`:
...
В `server/src/games/lucid/generation/schema.test.ts` добавить, повторяя вызовы фабрик в том виде, в каком они уже используются в этом файле:

```ts
  it('принимает мир с краями', () => {
  ...
  it('отвергает край без цвета и с кривым цветом', () => {
    const world = validWorld();
  ...
  it('отвергает два края и шесть краёв', () => {
    const region = { name: 'Край', color: '#c9b79c' };

    expect(worldSchema.safeParse({
      ...validWorld(),
      theme: { ...validWorld().theme, regions: [region, region] },
    }).success).toBe(false);
    expect(worldSchema.safeParse({
      ...validWorld(),
      theme: { ...validWorld().theme, regions: Array.from({ length: 6 }, () => region) },
    }).success).toBe(false);
  });
```

- [ ] **Шаг 3: Добавить края в схему мира**

В `server/src/games/lucid/generation/schema.ts` — поле в схеме темы. Используется zod 4: `z.strictObject`, `z.enum(TSEnum)`, `z.nativeEnum` устарел.

```ts
const REGION_RANGE = { min: 3, max: 5 } as const;
const REGION_NAME_MAX = 24;
// Шесть шестнадцатеричных цифр: короткую запись не принимаем, чтобы на клиенте
// был ровно один формат
const HEX_COLOR = /^#[\da-f]{6}$/i;
```

Поле делается **обязательным в схеме, но необязательным в типе**. Причина: провайдеру нужно требование, иначе модель поле просто не вернёт — это уже видели с настроением темы. А тип обязан допускать отсутствие, потому что партии, сохранённые до этой правки, лежат в базе без краёв и обязаны продолжать играться.

- [ ] **Шаг 4: Строки в промпт**

В `server/src/games/lucid/generation/prompt.ts` — края в образец JSON (иначе модель их не вернёт) и объяснение рядом с остальными:

- три-пять краёв, по порядку следования от старта к финишу;
- название короткое, не длиннее двадцати четырёх знаков, в духе мира: «Соляные пустоши», «Машинный зал», а не «Край номер два»;
- цвет каждого края различим от соседних и **перекликается с палитрой мира**, а не спорит с ней;
- края описывают места, а не события: что за местность, а не что там случится.

Последнее важно: содержимое клеток скрыто до посещения, и край, названный «Ловушки», обещал бы игроку то, чего он знать не должен.

- [ ] **Шаг 5: Края у запасной партии**

В `server/src/games/lucid/generation/fallback.json` добавить теме три-четыре края в духе «Заброшенной станции». Запасная партия — это та, в которую играют, когда генерация не удалась; выглядеть беднее настоящей она не должна.
...
```bash
git add tools/shared server/src/games/lucid
git commit -m "feat(lucid): добавить миру края с названиями и цветами"
```

---

## Задача 19: Поле как игровая доска

Заменяет собой облик, сделанный в задаче 12. Механика раскладки, вывод ролей цвета, фон и шрифты остаются; меняется то, как путь нарисован.

Заказчик посмотрел на готовое поле и сказал: скучно. Он прав, и причина не в исполнении. Схема маршрутов — язык, придуманный для того, чтобы **быстро понять**, а не чтобы **было интересно смотреть**. Для пересадки в метро это ровно то, что нужно; для вечера с друзьями — нет.

Что на бумажной доске создаёт ощущение, которого нам не хватает, и что из этого достижимо без картинок:

| На доске | У нас |
|---|---|
| путь — лента из крупных плиток | делаем: клетка становится плиткой на ленте |
| путь извивается | делаем: изгиб выводится из названия мира |
| цвет клеток меняется | делаем через края мира (задача 18) |
| дорога идёт через разные места | делаем: фон за участком подкрашен, край подписан |
| рисованные человечки и сундуки | **не делаем**: от генерации картинок отказались осознанно (ADR-0001) |

Последнюю строку стоит держать в голове: обаяние бумажной доски во многом держится на рисунках, и полностью мы его не повторим. Задача — уйти от схемы к игровой доске, а не догнать иллюстратора.

**Файлы:**
- Изменить: `docs/lucid/DESIGN.md`
- Изменить: `client/src/lib/lucid/trackLayout.ts`
- Изменить: `client/src/lib/lucid/trackLayout.test.ts`
- Создать: `client/src/lib/lucid/regions.ts`
- Создать: `client/src/lib/lucid/regions.test.ts`
- Изменить: `client/src/lib/lucid/colors.ts`
- Изменить: `client/src/routes/games/lucid/PartyPage/components/Board/index.tsx`
- Изменить: `client/src/routes/games/lucid/PartyPage/components/Board/linkPath.ts`
- Изменить: `client/src/routes/games/lucid/PartyPage/components/Board/linkPath.test.ts`
- Изменить: `client/src/routes/games/lucid/PartyPage/components/Board/Board.stories.tsx`

- [ ] **Шаг 1: Поле занимает экран**

Сейчас число клеток в ряду считается только от ширины (`cellsPerRow`), поэтому поле висит узкой лентой посреди пустого экрана: на десктопе сверху и снизу пусто, на телефоне пусто ещё сильнее. В `DESIGN.md` записано обратное — поле занимает экран целиком и является его героем.

Заменить `cellsPerRow(width)` на функцию, которая принимает **ширину, высоту и длину пути** и подбирает число клеток в ряду так, чтобы пропорции поля совпали с пропорциями контейнера:
...
export const cellsPerRowFor = (width: number, height: number, depth: number): number => {
  ...
  for (let perRow = MIN; perRow <= MAX; perRow++) {
    const rows = Math.ceil(depth / perRow);
...
Тесты: на узком высоком контейнере рядов больше, чем на широком низком; результат всегда в пределах от четырёх до четырнадцати; короткий путь не даёт деления на ноль.

- [ ] **Шаг 2: Путь извивается**

Убрать повороты кратно 45° — они принадлежат языку схемы, от которого мы уходим. Путь становится гладкой кривой, проходящей через центры клеток.

Каждой клетке добавляется смещение по вертикали, выведенное из её места в пути и из хеша названия мира:
...
const meander = (depth: number, seed: number): number => {
  ...
  const frequency = 0.6 + ((seed >> 8) % 5) / 10;

  return Math.sin(depth * frequency + phase) * MEANDER_AMPLITUDE;
};
```

`MEANDER_AMPLITUDE` — около 0.18 от шага ряда: заметно глазу и не съедает зазор между рядами.

Линия между соседними клетками рисуется гладко (`C`-кривая Безье с горизонтальными касательными либо Catmull-Rom, приведённая к кубической), развороты в конце ряда — широкой дугой, а не скосом. Пряди развилки по-прежнему расходятся и сходятся; расхождение тоже сглаживается.

Толщина ленты одинакова везде: путь равноправен на всём протяжении, это правило остаётся.

- [ ] **Шаг 3: Клетка становится плиткой**

Вместо кружка радиуса 14 — скруглённый прямоугольник поперёк ленты, повёрнутый по касательной к пути в этой точке. Размер порядка 34 на 26 единиц.

- Непройденная плитка: заливка цветом своего края, приглушённая до фона.
- Пройденная (`state.G.visited`): заливка цветом края в полную силу и обводка.
- Старт и финиш: плитка крупнее и другой формы, как сейчас, — у пути есть начало и конец, и это видно без подписей.

Фишки едут **по ленте**, поверх плиток, а не рядом.

- [ ] **Шаг 4: Края мира на поле**
...
Если краёв нет (старая партия), путь красится одним цветом линии, как сейчас, и подписей нет — отсутствие поля обрабатывается, а не роняет экран.

На поле край даёт три вещи:

1. Цвет плиток и ленты на своём отрезке.
2. Мягкую подложку за отрезком — очень низкий контраст к основе, это воздух, а не второй предмет внимания.
3. Подпись названия один раз на край, у его начала, мелко.

**Цвета краёв приходят от модели и потому произвольны.** Каждый обязан пройти ту же обработку, что и остальные роли: лента — не ниже 3:1 к основе, подпись — не ниже 4.5:1. Используй `ensureContrast`, уже вынесенный в `colors.ts`. Тест в `regions.test.ts` гоняет недружественные цвета краёв — одинаковые, чёрные, белые, совпадающие с основой — и требует соблюдения обоих порогов.

- [ ] **Шаг 5: Привести DESIGN.md в соответствие**

В `docs/lucid/DESIGN.md` раздел «Поле» сейчас описывает язык схемы маршрутов и повороты кратно 45°, а раздел «Цвет» утверждает, что смысл не кодируется цветом. Первое заменено, второе требует уточнения: цвет по-прежнему не кодирует опасность или выгоду, но теперь кодирует **место** — край мира. Перепиши оба места под то, как сделано, и объясни, почему ушли от схемы: она отвечает на «быстро понять», а нужно «интересно смотреть».

Список «чего намеренно не делаем» не трогай — он остаётся в силе.

- [ ] **Шаг 6: Обновить истории**

Пять существующих историй должны пережить переделку. Добавить к ним две:

6. **NoRegions** — тема без краёв: партия из базы, сгенерированная до этой правки. Поле обязано рисоваться.
7. **HostileRegions** — края с цветами, совпадающими с основой и друг с другом. Поле обязано читаться.

В остальных историях заполнить края правдоподобно: «Заброшенная станция» — машинный зал, оранжерея, соляные пустоши.

- [ ] **Шаг 7: Проверить**

Выполнить: `yarn workspace @trgames/client lint`

Затем глазами в Storybook: на широком экране поле занимает его целиком, на 360 пикселях — тоже, горизонтальной прокрутки нет, подписи краёв читаются, недружественные цвета не ломают поле.

- [ ] **Шаг 8: Коммит**

```bash
git add client/src docs/lucid
git commit -m "feat(lucid): переделать поле из схемы маршрутов в игровую доску"
```

---

### Принятый облик: грубые изразцы

Заказчик выбрал вариант из пяти показанных макетов — «грубые изразцы»: плитка почти без скругления, с жёстким краем. Ближе к бумажной доске, дальше от экранной схемы. Смотреть макеты: <https://claude.ai/artifact/9ENcr5V9LfwAkxbL4fRnto>, артборд **Chunky**.

Он сказал «начнём с этого, потом может доработаем визуал» — значит числа ниже не священны, но отступать от них надо осознанно и говорить об этом.

Числа из принятого макета, в условных единицах раскладки:

| Что | Значение |
|---|---|
| шаг клетки по горизонтали | 132 |
| шаг ряда | 150 |
| отступ пряди развилки от общей линии | 40 |
| размах изгиба | 26 |
| частота изгиба | `0.55 + (хеш мира >> 8) % 5 / 10` |
| фаза изгиба | `(хеш мира % 360)` градусов |
| толщина ленты | 30 |
| плитка события | 36 вдоль пути на 28 поперёк |
| плитка старта и финиша | 42 на 42 |
| скругление плитки | обводка 2 с `stroke-linejoin="round"` — почти угол |
| подложка края | прозрачность 0.07 |
| лента на участке края | прозрачность 0.55 |
| подпись края | 15 пикселей, прозрачность 0.85 |
| клеток в ряду на телефоне | 4 при ширине 390 |

Приёмы, на которых держится макет:

**Плитка повёрнута по касательной к пути.** Угол берётся по соседям той же пряди: `atan2(следующая.y − предыдущая.y, следующая.x − предыдущая.x)`. На развороте в конце ряда горизонтальная разница обращается в ноль и угол теряет смысл — там плитка остаётся прямой.

**Скругление делается обводкой, а не радиусом.** Плитка рисуется четырёхугольником и обводится собственным цветом с `stroke-linejoin="round"`; толщина обводки и есть степень скругления. Два — почти угол, девять — мягкая плитка. Это позволяет собрать все плитки одного вида в один `<path>` со множеством подпутей вместо сотни элементов.

**Лента режется по краям.** Связи группируются по краю своей начальной клетки, и каждая группа рисуется своим цветом. Один `<path>` на край.

**Связь между клетками — кубическая кривая.** Внутри ряда управляющие точки отнесены на половину горизонтальной разницы, отчего получается пологая S. На переходе между рядами горизонтальной разницы нет: управляющие точки выносятся наружу на 70 единиц в ту сторону, где край ряда, и выходит широкая дуга вместо скоса.

**Непройденная плитка** заливается своим цветом с прозрачностью 0.55 поверх основы, пройденная — 0.95. Обводка полной силы у обеих: границы клеток видны всегда, иначе путь рассыпается.

Что в макете намеренно не проверено и остаётся на реализацию: гарантии контраста (в макете цвета краёв подобраны руками, в игре придут от модели), заполнение экрана по высоте, прочерчивание пути, выбор ветки с клавиатуры, стопка ников на общей клетке.
