...
**Стек:** React 18, MobX, socket.io-client, Tailwind, Radix UI (через существующие `@/components/ui`), SVG для поля, WebAudio для звука, vitest для проверяемых гарантий.
...
### Звук: действия синтезируются, музыка — файлы, которых пока нет
Короткие звуки действий (кубик, шаг, ресурс, развилка, победа) синтезируются WebAudio и не требуют ассетов. Музыка — два файла по настроению темы; их должен дать заказчик. Пока файлов нет, музыкальный слой молчит и не роняет игру. Вопрос записан в `OPEN-QUESTIONS.md`.
...
В раздел «Вопросы к заказчику» добавить:

```markdown
### Музыкальных файлов нет
Звуки действий синтезируются на лету и ассетов не требуют. Музыка — два файла,
светлый и тёмный, по настроению темы. Их неоткуда взять: сгенерировать нельзя
(решение по звуку — заранее записанные файлы), а свободные библиотеки подбирать
на вкус заказчика бессмысленно.

Пока файлов нет, музыкальный слой молчит: игра работает, просто тихо. Нужны два
файла в `client/src/assets/games/lucid/music/` под именами `light.mp3` и
`dark.mp3`.
```
...
**Файлы:**
- Изменить: `client/index.html`
- Изменить: `client/tailwind.config.js`

- [ ] **Шаг 1: Подключить шрифты**

В `client/index.html` перед закрывающим `</head>` добавить:
...
```html
  ...
  <link
    rel="stylesheet"
    href="https://fonts.googleapis.com/css2?family=Golos+Text:wght@400;500;600&family=Unbounded:wght@400;600;800&display=swap"/>
```
...
Кнопка «Сыграть ещё тем же составом» отправляет `playAgain`. Создаётся новая партия, уже подключённые попадают в неё, тема выбирается заново, мир другой. Заново рассылать ссылку там, где все уже собраны, — лишнее.
...
- [ ] **Шаг 2: Звуки действий**

`LucidSoundService` синтезирует короткие звуки через WebAudio: ассетов они не требуют и грузить нечего.

Озвучиваются ровно шесть действий, остальное молчит — иначе выйдет не живо, а шумно:

| Событие | Звук |
|---|---|
| бросок кубика | короткая дробь из трёх щелчков |
| шаг фишки | мягкий тон, повторённый по числу пройденных клеток |
| открытие события | один низкий тон с затуханием |
| ресурс получен или потерян | восходящий и нисходящий интервал |
| выбор ветки | два тона подряд |
| победа | трезвучие |

Каждый звук — `OscillatorNode` с `GainNode` и затуханием; общий `GainNode` умножается на громкость из настроек.
...
Файлы ожидаются по путям `client/src/assets/games/lucid/music/light.mp3` и `dark.mp3`. **Их сейчас нет.** Загрузка делается так, чтобы отсутствие файла не роняло игру:

```ts
// Музыкальных файлов пока нет: заказчик их даст. До тех пор слой молчит,
// а не падает — партия играется и без музыки
private playMusic = (mood: 'dark' | 'light'): void => {
  const source = this.musicUrls[mood];
...
Подключать файлы динамическим импортом с перехватом ошибки, а не статическим: статический импорт отсутствующего файла ломает сборку.
...
- **Storybook для экранов lucid не пишется.** Экраны целиком зависят от присланного вида и сгенерированной палитры; отдельные истории для них стоили бы дороже пользы. Проверяемые части — цвет и раскладка — покрыты тестами.
