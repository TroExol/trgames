...
## Что изменилось после выполнения плана

План выполнен целиком, но код разошёлся с ним в нескольких местах. Ниже — перечень расхождений, чтобы документ не вводил в заблуждение. Источник правды по этим пунктам — код и `docs/lucid/CONTEXT.md`.

**Два дефекта движка, вскрытых сквозным тестом задачи 14.** Разыгранное событие переоткрывалось, если вариант не двигал игрока, и партия не заканчивалась никогда. Событие, все варианты которого игроку не по карману, оставляло его без единого допустимого хода. Оба исправлены: событие завершает ход, а недоступное целиком событие проходит мимо. Обе правки отражены в тексте задачи 7 выше.

**Провайдер модели.** Вместо прямого обращения к DeepSeek запросы идут через OpenRouter: модель задаётся переменной окружения, структурированный вывод запрашивается схемой, выведенной из zod-описаний, фактическая стоимость приходит в ответе и пишется в хранилище рядом с токенами. Файл `deepseek.ts` стал `model.ts`, в статистике использования появилось поле стоимости.

**Запасная партия.** Вместо шести событий — полный пул по числу клеток событий самого длинного трека. Раскладываются они не по фиксированным номерам, а по клеткам конкретного трека, перемешанные от сида: `loadFallbackContent` принимает номера клеток и сид, `generateContent` — сид. Вернулась функция перемешивания, удалённая в задаче 2 как неиспользуемая: тогда у неё честно не было вызовов.

**Повторы номеров клеток в ответе модели.** Пайплайн отбрасывает повторное событие для уже занятой клетки, а не перезаписывает прежнее: иначе одна клетка получала бы два события, другая оставалась пустой, а ответ считался бы успешным.

**Тесты.** Их стало 115 вместо запланированных 110: добавлены проверки пограничных случаев по итогам ревью, возвращены тесты перемешивания и добавлена проверка, что за партию событий действительно выпадает много — раньше сквозной тест убеждался только в том, что партия дошла до победителя.
